// GERENCIAMENTO — serviço próprio derivado do contrato do arquivo fornecido.
// Implantação ainda não executada. Não contém IDs ou URLs de outros aplicativos.
// Requer propriedades: GERENCIAMENTO_SHEET_ID, GERENCIAMENTO_ALLOWED_EMAILS,
// GERENCIAMENTO_OBRA_ACCESS, GERENCIAMENTO_IA_MODEL e ANTHROPIC_API_KEY
// (as duas últimas somente para IA).
// GERENCIAMENTO_ALLOWED_EMAILS: lista de e-mails autorizados a USAR o serviço.
// GERENCIAMENTO_OBRA_ACCESS: JSON {"email":["obraId1","obraId2"], "admin@x.com":["*"]}
// — define QUAIS obras cada e-mail pode acessar. Sem essa propriedade, ou sem
// o e-mail nela, o acesso é negado por padrão (nunca liberado por omissão).
// Publique executando como o usuário que acessa; acesso restrito aos usuários autorizados.
// Cada obra usa abas e pastas próprias. Não executar em planilha de outro aplicativo.
const SHEET_ID = PropertiesService.getScriptProperties().getProperty('GERENCIAMENTO_SHEET_ID');
let OBRA_ATIVA = '';
...
function doGet(e) {
  ...
  try {
    ...
    if (path === 'backup'  && action === 'buscar-ultimo') {
      return buscarUltimoBackup(OBRA_ATIVA);
    }
...
function doPost(e) {
  try {
    ...
    autorizarGerenciamento(e,body);
    const path   = (e && e.parameter && e.parameter.path)   || '';
    const action = (e && e.parameter && e.parameter.action) || '';
    if(path==='diario'&&action==='lista-mes')return listarDiariosMes(body.mes||'');
    if(path==='diario'&&action==='carregar')return carregarDiario(body.data||'');
    if(path==='diario'&&action==='limpar-duplicatas')return successResponse({ok:true,...limparDuplicatasDiario()});
...
function salvarDiario(payload) {
  ...
  let sheet = ss.getSheetByName(nomeAbaGerenciamento(SHEET_NAME_DIARIO));
  if (!sheet) sheet = ss.insertSheet(nomeAbaGerenciamento(SHEET_NAME_DIARIO));

  // Reescreve cabeçalho se necessário (planilha nova ou sem cabeçalho)
  const colA = sheet.getRange(1, 1).getValue();
  if (colA !== 'Data') {
    if(sheet.getLastRow()>1)throw new Error('Cabeçalho incompatível. Nenhum registro foi apagado.');
  ...
  const linha = [
    ...
    payload.efetivoTotal           ?? '',   // N Efetivo Total
...
function carregarDiario(data) {
  ...
  for (let i = 1; i < valores.length; i++) {
    if (normalizarData(valores[i][0]) === normalizarData(data)) {
      const diario = {};
...
function listarDiariosMes(mes) {
  ...
  const diarios  = [];
  for (let i = 1; i < valores.length; i++) {
    if (valores[i][0] && normalizarData(valores[i][0]).startsWith(mes)) {
      const d = {};
...
function salvarFoto(dados) {
  ...
  var arquivo = pastaData.createFile(Utilities.newBlob(bytes, dados.mime||'image/jpeg', dados.nome||nome));
  // O arquivo conserva acesso privado do serviço do Gerenciamento.
...
// ============================================================
var PASTA_RAIZ_BACKUPS = 'Gerenciamento - Backups';
...
var MODELO_IA_PERGUNTAS = PropertiesService.getScriptProperties().getProperty('GERENCIAMENTO_IA_MODEL');
...
function chamarClaude(pergunta, contextoJson, apiKey) {
  var systemPrompt =
    ...
    'EXCLUSIVAMENTE nos dados JSON fornecidos abaixo (vindos dos registros do Gerenciamento da obra autorizada). ' +
...
function getSheet(name) {
  const ss=SpreadsheetApp.openById(SHEET_ID);const title=nomeAbaGerenciamento(name);let sh=ss.getSheetByName(title);if(!sh){sh=ss.insertSheet(title);const headers=name===SHEET_NAME_PAUTA?['id','assunto','descrição','criador','responsável','setor','prioridade','status','data_lançamento','data_término','criado_em','atualizado_em']:name===SHEET_NAME_CHECKIN?['id','data','hora','obra','assuntos','resumo','criado_em']:COLUNAS_DIARIO;sh.appendRow(headers);}return sh;
}
...
function autorizarGerenciamento(e,body){
 if(!SHEET_ID)throw new Error('Configure uma planilha exclusiva em GERENCIAMENTO_SHEET_ID.');
 const email=Session.getActiveUser().getEmail();
 const allowed=(PropertiesService.getScriptProperties().getProperty('GERENCIAMENTO_ALLOWED_EMAILS')||'').split(',').map(x=>x.trim().toLowerCase()).filter(Boolean);
 if(!email||!allowed.includes(email.toLowerCase()))throw new Error('Usuário não autorizado no Gerenciamento.');
 OBRA_ATIVA=String(body?.obra_id||e?.parameter?.obra_id||'');
 if(!/^[a-zA-Z0-9_-]{1,64}$/.test(OBRA_ATIVA))throw new Error('Informe a obra válida do Gerenciamento.');
 verificarAcessoObra(email,OBRA_ATIVA);
}

// Confere se o e-mail autenticado tem permissão para a obra pedida.
// Estar na lista GERENCIAMENTO_ALLOWED_EMAILS só autoriza o uso do serviço;
// esta função é quem decide QUAL obra esse e-mail pode acessar. Sem
// configuração de acesso por obra, o padrão é negar — nunca liberar tudo
// por omissão.
function verificarAcessoObra(email, obraId){
 const raw=PropertiesService.getScriptProperties().getProperty('GERENCIAMENTO_OBRA_ACCESS');
 if(!raw){
   throw new Error('Acesso por obra não configurado (GERENCIAMENTO_OBRA_ACCESS). Nenhuma obra é liberada até essa propriedade existir.');
 }
 let mapa;
 try{ mapa=JSON.parse(raw); }
 catch(err){ throw new Error('GERENCIAMENTO_OBRA_ACCESS inválido: precisa ser um JSON como {"email":["obra1","obra2"]}.'); }

 const chave=Object.keys(mapa).find(k=>k.toLowerCase()===email.toLowerCase());
 const obrasDoUsuario=chave?mapa[chave]:null;
 if(!Array.isArray(obrasDoUsuario)){
   throw new Error('Usuário sem obras liberadas em GERENCIAMENTO_OBRA_ACCESS.');
 }
 const liberado = obrasDoUsuario.includes('*') || obrasDoUsuario.includes(obraId);
 if(!liberado){
   throw new Error('Usuário não tem acesso à obra "'+obraId+'".');
 }
}
function nomeAbaGerenciamento(name){if(!OBRA_ATIVA)throw new Error('Obra não definida.');return name+'_'+OBRA_ATIVA;}
