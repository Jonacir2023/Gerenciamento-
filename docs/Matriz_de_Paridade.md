...
| Remover pauta | `removerAssunto`: só local, **nunca excluía na planilha** — o registro remoto ficava "vivo" para sempre | `excluirPautaGoogle` chama `path=pauta&action=excluir` (novo endpoint no backend, exclusão lógica) | **Gap do original fechado nesta sessão** — ver `docs/VALIDACAO.md`, rodada de sincronização | Smoke test com stub HTTPS local, POST confirmado |
...
| Criar/editar pelo Cadastro do Check-in | `salvarAssunto()`: **nenhuma sincronização remota** — criar ou editar um assunto pela aba Cadastro do Check-in nunca chegava à planilha (só a mudança de status pelo Kanban, via `moverCard`) — bug já presente no original, não introduzido pela adaptação | `criarAssuntoGoogle`/`atualizarAssuntoGoogle`, chamados após salvar local; ID atribuído pelo servidor é adotado de volta, igual ao que `pauta.html` já fazia para criação | **Gap do original fechado nesta sessão** | Smoke test com stub HTTPS local: POST `criar` e `atualizar` confirmados com o payload real do formulário |
| Remover pelo Cadastro do Check-in / remover na Pauta | `removerAssunto()`: só local nos dois apps — nunca excluía na planilha, o registro remoto ficava "vivo" para sempre | `excluirAssuntoGoogle`/`excluirPautaGoogle` chamam o novo endpoint `path=pauta&action=excluir` (exclusão lógica no backend, coluna "Excluído") | **Gap do original fechado nesta sessão** | Smoke test: POST `excluir` confirmado nos dois módulos |
